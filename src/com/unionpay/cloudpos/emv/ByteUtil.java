...
class ByteUtil
{
    ...
    static void asciiToBCD(byte[] ascii_buf, int asc_offset, byte[] bcd_buf, int bcd_offset,
    ...
    static byte[] bcdToAscii(byte[] bcdByte)
    {
    ...
    static byte[] bcdToAscii(byte[] bcdByte, int offset, int length)
    {
    ...
    static void bcdToAscii(byte[] bcd_buf, int offset, byte[] ascii_buf, int asc_offset,
            int conv_len, int type)
    {
        int cnt;
        ...
        for (; cnt < conv_len; cnt++, asciiOffset++)
        {
            ...
            ascii_buf[asciiOffset] += (byte) ((ascii_buf[asciiOffset] > 9) ? (65 - 10) : 48); // 65
                                                                                              // =
                                                                                              // 'A'
                                                                                              // 48
                                                                                              // =
    ...
    static int bcdToInt(byte[] bcdByte) // 嵌套调用的处理过程太复杂... DuanCS@[20140625]
    {
     ...
     * @param tag the second byte array against.
     * @return <code>true</code> if the <code>String </code>are equal;
     *         <code>false</code> otherwise.
     */
    static boolean equalByteArray(byte[] src, byte[] dst)
    {
        return equalByteArray(src, 0, src.length, dst, 0, dst.length);
    }

    static boolean equalByteArray(byte[] src, int srcOffset, int srcLen, byte[] dst, int dstOffset,
            int dstLen)
    {
    ...
    static int compareByteArray(byte[] src, byte[] dst)
    {
        return compareByteArray(src, 0, src.length, dst, 0, dst.length);
    }

    static int compareByteArray(byte[] src, int srcOffset, byte[] dst, int dstOffset, int length)
    {
     ...
     * @param dstOffset the start position of the second byte array
     ...
     */
    static int compareByteArray(byte[] src, int srcOffset, int srcLen, byte[] dst, int dstOffset,
            int dstLen)
    {
        char c1, c2;
    ...
    static void arraycopy(String src, int srcOffset, byte[] dst, int dstOffset, int length) {
    ...
    static byte[] getLengthArray(int srcLength) {
        byte[] destArray = null;
        if (srcLength <= 0x7F) {
    ...
    static String arrayToHexStr(byte[] src)
    {
        return arrayToHexStr(null, src, 0, Integer.MAX_VALUE);
    }

    static String arrayToHexStr(byte[] src, int srcLen)
    {
        return arrayToHexStr(null, src, 0, srcLen);
    }

    static String arrayToHexStr(byte[] src, int offset, int srcLen)
    {
        return arrayToHexStr(null, src, offset, srcLen);
    }

    static String arrayToHexStr(String prefix, byte[] src)
    {
        return arrayToHexStr(prefix, src, 0, Integer.MAX_VALUE);
    }

    static String arrayToHexStr(String prefix, byte[] src, int srcLen)
    {
        return arrayToHexStr(prefix, src, 0, srcLen);
    }

    static String arrayToHexStr(String prefix, byte[] src, int offset, int srcLen)
    {
        int prefixLen = prefix == null ? 0 : prefix.length();

        srcLen = (src == null || offset < 0 || srcLen < 0) ? offset = 0
                : (src.length < srcLen) ? src.length : srcLen;
    ...
    static int getIndexOf(byte value, byte[] items) { // 查value在items中的索引
