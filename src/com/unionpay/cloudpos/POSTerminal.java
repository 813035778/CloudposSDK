...
public abstract class POSTerminal {
	...
	public static synchronized POSTerminal getInstance(Context context) {
		...
		if (self == null) {
			...
			if (self == null) {
				...
				getPOSTerminalInternalClass(internalclassName);
				if (self == null) {
	 ...
	 * @return void
	 */
	private static void getPOSTerminalClass(String className) {
		try {
			Object terminalObj = null;
			File dexOutputDir = androidContext.getDir("dex", Context.MODE_PRIVATE);
			DexClassLoader dexClassLoader = new DexClassLoader(LOAD_JAR_PATH, dexOutputDir.getAbsolutePath(), null,
					POSTerminal.class.getClassLoader());// success
	 ...
	 * @return void
	 */
	private static void getPOSTerminalInternalClass(String internalclassName) {
		try {
			Object terminalObj = null;
			...
			//     				e.printStackTrace();
			Log.i("获取POSTerminal", "无法从系统classpath中获取" + internalclassName);
		}
	}
